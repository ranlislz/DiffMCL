...
class DiffMCL:
    ...
    def __init__(self):
        ...
        self.device = torch.device(self.args.device if self.args.cuda and torch.cuda.is_available() else "cpu")
    ...
    def parse_options(parser):
        ...
        parser.add_argument("--unsup_mode", type=str, default="name", help="unsup mode")
        ...
        parser.add_argument("--diff_dim", type=int, default=64, help="the hidden size of diffusion latent space")
        parser.add_argument("--num_layer", type=int, default=4, help="number of layers for diffusion model")
        ...
        parser.add_argument("--scale", type=float, default=0, help="scale of perturbation")
        parser.add_argument("--num_aug", type=int, default=1, help="number of augmentation")
    ...
    def init_model(self):
        ...
        #diff_dim = self.args.diff_dim
        self.diffusion = DenoisingDiffusion(self.args.diff_dim, self.args.diff_dim, 4, self.device).to(self.device)
        self.multi_loss_layer = CustomMultiLossLayer(loss_num=self.args.inner_view_num).to(self.device)
        self.align_multi_loss_layer = CustomMultiLossLayer(loss_num=self.args.inner_view_num + 2).to(self.device)
        self.criterion_diff = nn.MSELoss().to(self.device)
        #self.multi_loss_layer_aug = CustomMultiLossLayer(loss_num=self.args.inner_view_num+1).to(self.device)
        #self.align_multi_loss_layer_aug = CustomMultiLossLayer(loss_num=self.args.inner_view_num + 1).to(self.device)
        ...
        self.params = [
            {"params":
                 ...
                 list(self.criterion_diff.parameters()) +
        ...
        total_params += sum(p.numel() for p in self.criterion_diff.parameters() if p.requires_grad)
        total_params += sum(p.numel() for p in self.diffusion.parameters() if p.requires_grad)
    ...
    def semi_supervised_learning(self):
        ...
        with torch.no_grad():
            ...
            name_emb, char_emb, _ , _ , joint_emb = self.multimodal_encoder(self.input_idx,
                                                                    self.adj,
                                                                    self.img_features,
                                                                    self.rel_features,
                                                                    self.att_features,
    ...
    def train(self):
        ...
        with tqdm(total=self.args.epochs, desc='(T)') as pbar:
            for epoch in range(self.args.epochs):
                ...
                for i in range(num_aug):
                    if i == 0:
                        joint_emb_aug = joint_emb + torch.randn_like(joint_emb) * scale
                        topo_emb_aug = topo_emb + torch.randn_like(topo_emb) * scale
                        sema_emb_aug = sema_emb + torch.randn_like(sema_emb) * scale
                        gph_emb_aug = self.diffusion(gph_emb, noise_scale, num_steps)
                        img_emb_aug = self.diffusion(img_emb, noise_scale, num_steps)
                        rel_emb_aug = self.diffusion(rel_emb, noise_scale, num_steps)
                        att_emb_aug = self.diffusion(att_emb, noise_scale, num_steps)
                        name_emb_aug = self.diffusion(name_emb, noise_scale, num_steps) if name_emb is not None else name_emb
                        char_emb_aug = self.diffusion(char_emb, noise_scale, num_steps) if char_emb is not None else char_emb
                        emb_aug_list.append((gph_emb_aug, img_emb_aug, rel_emb_aug, att_emb_aug, name_emb_aug, char_emb_aug, topo_emb_aug, sema_emb_aug,
                                              joint_emb_aug))
                    elif i == 1:
                        torch.manual_seed(i)
                        gph_emb_aug = gph_emb + torch.randn_like(gph_emb) * scale if gph_emb is not None else 0
                        img_emb_aug = img_emb + torch.randn_like(img_emb) * scale if img_emb is not None else 0
                        rel_emb_aug = rel_emb + torch.randn_like(rel_emb) * scale if rel_emb is not None else 0
                        att_emb_aug = att_emb + torch.randn_like(att_emb) * scale if att_emb is not None else 0
                        name_emb_aug = att_emb + torch.randn_like(name_emb) * scale if name_emb is not None else name_emb
                        char_emb_aug = char_emb + torch.randn_like(char_emb) * scale if char_emb is not None else char_emb
                        joint_emb_aug = joint_emb + torch.randn_like(joint_emb) * scale
                        topo_emb_aug = topo_emb + torch.randn_like(topo_emb) * scale
                        sema_emb_aug = sema_emb + torch.randn_like(sema_emb) * scale
                        emb_aug_list.append((gph_emb_aug, img_emb_aug, rel_emb_aug, att_emb_aug, name_emb_aug,
                                             char_emb_aug, topo_emb_aug, sema_emb_aug,
                                             joint_emb_aug))
                # add original embeddings to the list of augmented embeddings
                # emb_aug_list.insert(0, (gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb, topo_emb, sema_emb, joint_emb))
                ...
                for si in np.arange(0, self.train_ill.shape[0], bsize):
                    ...
                    for i, (gph_emb_aug, img_emb_aug, rel_emb_aug, att_emb_aug, name_emb_aug, char_emb_aug, topo_emb_aug, sema_emb_aug, joint_emb_aug) in enumerate(
                            emb_aug_list):
                        ...
                        if i == 0:
                            loss_diff = self.criterion_diff(gph_emb,gph_emb_aug) + self.criterion_diff(img_emb,img_emb_aug) + \
                                         self.criterion_diff(rel_emb,rel_emb_aug) + self.criterion_diff(att_emb,att_emb_aug) #+ self.criterion_diff(joint_emb,joint_emb_aug)
                            if name_emb is not None:
                                loss_diff += self.criterion_diff(name_emb, name_emb_aug)
                            if char_emb is not None:
                                loss_diff += self.criterion_diff(char_emb, char_emb_aug)

                        in_loss_aug = self.inner_view_loss(gph_emb_aug, rel_emb_aug, att_emb_aug, img_emb_aug, name_emb_aug,
                                                           char_emb_aug, self.train_ill[si:si + bsize])
                        ...
                        if i == 2:
                            ...
                            losses.append((loss_joi_aug + in_loss_aug))
                        # sum the losses for all versions of augmented embeddings
                    loss_all = sum(losses)
                ...
                del joint_emb, gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb, topo_emb , sema_emb
                pbar.set_postfix({'loss': loss_sum_all})
                pbar.update()
    ...
    def test(self, epoch):
        with torch.no_grad():
            ...
            if "100" in self.args.file_dir:
                ...
                del distance, gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb, topo_emb , sema_emb, joint_emb
                gc.collect()
            ...
            print("r2l: acc of top {} = {}, mr = {:.3f}, mrr = {:.3f}, time = {:.4f} s \n".format(top_k, acc_r2l,
                                                                                                  ...
                                                                                                  time.time() - t_test))
            if epoch + 1 == self.args.epochs:
                l2r,r2l = [top_k, acc_l2r, mean_l2r, mrr_l2r], [top_k, acc_r2l, mean_r2l, mrr_r2l]

        return acc_r2l[0], acc_l2r[0]
